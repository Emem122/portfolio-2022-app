          ...
          {aboutData.map((data) => {
            return (
              <div key={data.id} className={style.aboutGridContent}>
                <h3 className="mb-2 text-xl font-bold text-clrPrimary md:text-2xl">
                  {data.title}
                </h3>
                <div
                  className={`${data.img && "flex flex-col gap-3 sm:flex-row"}`}
                >
                  {data.img && (
                    <div className="h-[150px] w-[150px] bg-gray-400">img</div>
                  )}
                  <div>
                      ...
                      return (
                        <div key={index} className="mb-2 last-of-type:mb-0">
                          <div className="flex items-start gap-2">
                            <div className="relative mt-1 h-[15px] w-[15px] shrink-0 rounded-sm border-[1px] border-clrBlack md:h-[20px] md:w-[20px]">
                              {list.checked && (
                                <svg
